	...
	def start_game(self):
		...
		while True:
			...
			if not stop:
				...
				player_total = self.sum_card_values(player)
				computer_total = self.sum_card_values(computer)
			...
			total = self.sum_card_values(player)
			if total > 21:
				...
				print("Stali ste s igrom. Završni rezultat je: Igrač {} vs Računalo {}".format(total, self.sum_card_values(computer)))
				break
